...
type Blog = { id?: number | null; slug?: string | null; title?: string | null; url?: string | null; featured_image_url?: string | null };
...
export default function MapScreen() {
  ...
  const [blogImageBySlug, setBlogImageBySlug] = useState<Record<string, string>>({});
  const blogFetchAbortRef = useRef<AbortController | null>(null);

  const [viewMode, setViewMode] = useState<"map" | "list">("map");
  const [filters, setFilters] = useState<Filters>(DEFAULT_FILTERS);
  const [filtersOpen, setFiltersOpen] = useState(false);
  const [expandedFilter, setExpandedFilter] = useState<null | "openNow" | "visitedOnly" | "access" | "courtType" | "courtsBucket" | "skill" | "sortBy">(null);
  ...
  // Fetch linked blog featured image for the selected location (details modal)
  useEffect(() => {
    const slug = selected?.blog?.slug ? String(selected.blog.slug).trim() : "";
    if (!slug) return;

    // If API already provided the image, no need to fetch.
    const direct = String((selected.blog as any)?.featured_image_url ?? "").trim();
    if (direct) {
      setBlogImageBySlug((prev) => (prev[slug] === direct ? prev : { ...prev, [slug]: direct }));
      return;
    }

    if (blogImageBySlug[slug]) return;

    blogFetchAbortRef.current?.abort();
    const ac = new AbortController();
    blogFetchAbortRef.current = ac;

    (async () => {
      try {
        const candidates = [
          apiUrl(`/api/v1/post.php?slug=${encodeURIComponent(slug)}`),
          apiUrl(`/api/v1/posts.php?slug=${encodeURIComponent(slug)}`),
        ];
        ...
        for (const u of candidates) {
          ...
          const post = (j.post ?? j.data?.post ?? j) as any;
          const img = String(post?.featured_image_url ?? "").trim();
          if (img) {
            setBlogImageBySlug((prev) => ({ ...prev, [slug]: img }));
            return;
          }
        }
      } catch (e: any) {
        ...
        if (blogFetchAbortRef.current === ac) blogFetchAbortRef.current = null;
      }
    })();
  ...
  // List pagination (10 at a time)
  useEffect(() => {
    setListLimit(10);
  }, [q, filters]);

  const listData = useMemo(() => filtered.slice(0, listLimit), [filtered, listLimit]);
...
function onPressNearMe() {
  (async () => {
    try {
      const { status } = await Location.requestForegroundPermissionsAsync();
      if (status !== "granted") {
        Alert.alert("Location", "Allow location so we can center the map near you.");
        return;
      }

      // Always fetch a fresh location when the user taps "Near me"
      ...
      // Radius-based deltas so the visible map roughly matches your radius
      const delta = clamp((radiusMiles / 69) * 2, 0.05, 8);
      const r: Region = {
        latitude: lat,
        longitude: lng,
        latitudeDelta: delta,
      ...
      fetchNearby(lat, lng, radiusMiles, false).catch((e) => Alert.alert("Error", e?.message ?? "Failed"));
    } catch (e: any) {
      Alert.alert("Location", e?.message ?? "We couldn’t get your location. Try again.");
    }
  })();
}
          ...
          ListFooterComponent={
            filtered.length > listLimit ? (
              <Pressable
                ...
                onPress={() => setListLimit((n) => Math.min(n + 10, filtered.length))}
              >
          ...
          {selected ? (
            <DetailsPanel
              ...
              featuredImageUrl={(() => {
                ...
                if (slug && blogImageBySlug[slug]) return blogImageBySlug[slug];
                return "";
              })()}
              onOpenBlog={() => {
        ...
        <SafeAreaView style={styles.modalSafe} edges={["top", "left", "right"]}>
          ...
          <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
            ...
            <ExpandableSelectRow
              ...
              expanded={expandedFilter === "access"}
              onToggle={() => setExpandedFilter((k) => (k === "access" ? null : "access"))}
              options={[
                { label: "All", value: "any" },
                { label: "Free/Public", value: "public" },
                { label: "Membership/Paid", value: "paid" },
              ]}
              selectedValue={filters.access}
              onSelect={(val) => setFilters((f) => ({ ...f, access: val as any }))}
            />

            <ExpandableSelectRow
              label="Court type"
              value={filters.courtType === "any" ? "Any" : filters.courtType === "indoor" ? "Indoor" : "Outdoor"}
              expanded={expandedFilter === "courtType"}
              onToggle={() => setExpandedFilter((k) => (k === "courtType" ? null : "courtType"))}
              options={[
                { label: "Any", value: "any" },
              ...
              onSelect={(val) => setFilters((f) => ({ ...f, courtType: val as any }))}
            />

            <ExpandableSelectRow
              label="Court count"
              value={filters.courtsBucket === "any" ? "Any" : filters.courtsBucket}
              expanded={expandedFilter === "courtsBucket"}
              onToggle={() => setExpandedFilter((k) => (k === "courtsBucket" ? null : "courtsBucket"))}
              options={[
                { label: "Any", value: "any" },
                { label: "1-2", value: "1-2" },
                { label: "3-5", value: "3-5" },
              ...
              selectedValue={filters.courtsBucket}
              onSelect={(val) => setFilters((f) => ({ ...f, courtsBucket: val as any }))}
            />

            <ExpandableMultiSelectRow
              label="Skill level"
              value={filters.skillLevels.length ? `${filters.skillLevels.length} selected` : "Any"}
              expanded={expandedFilter === "skill"}
              onToggle={() => setExpandedFilter((k) => (k === "skill" ? null : "skill"))}
              options={(["beginner", "intermediate", "adv_intermediate", "advanced", "pro"] as const).map((k) => ({
                label: SKILL_LABELS[k],
                value: k,
              }))}
              selectedValues={filters.skillLevels}
              onToggleValue={(val) =>
                setFilters((f) => ({
                  ...f,
                  skillLevels: f.skillLevels.includes(val as any)
                    ? f.skillLevels.filter((x) => x !== (val as any))
                    : [...f.skillLevels, val as any],
                }))
              }
              onClear={() => setFilters((f) => ({ ...f, skillLevels: [] }))}
            />

            <ExpandableSelectRow
              label="Sort by"
              value={filters.sortBy === "distance" ? "Distance" : filters.sortBy === "rating" ? "Rating" : "Courts"}
              expanded={expandedFilter === "sortBy"}
              onToggle={() => setExpandedFilter((k) => (k === "sortBy" ? null : "sortBy"))}
              options={[
...
function ResultCard({ item, onPress }: { item: LocationItemUI; onPress: () => void }) {
  ...
  return (
    <Pressable style={styles.card} onPress={onPress}>
      {(() => {
        const img = String(item.blog?.featured_image_url ?? "").trim();
        if (!img) return null;
        return <Image source={{ uri: img }} style={styles.cardImage} resizeMode="cover" />;
      })()}
...
{(() => {
  const hasBlog = !!(item.blog?.slug || item.blog?.url);
  const hasYouTube = !!String(item.youtube_url ?? "").trim();
  ...
  return (
    <View style={styles.cardCtaRow}>
      {hasBlog ? (
        <Pressable
          style={styles.cardCtaPrimary}
          onPress={() => {
            const slug = item.blog?.slug ? String(item.blog.slug) : "";
            if (slug) {
              try {
                router.push(`/blog/${slug}` as any);
                return;
            ...
            const u = item.blog?.url ? String(item.blog.url) : "";
            if (u) Linking.openURL(u);
          }}
        >
      ...
      {hasYouTube ? (
        <Pressable style={styles.cardCtaYouTube} onPress={() => Linking.openURL(String(item.youtube_url))}>
          <Ionicons name="logo-youtube" size={16} color="white" />
          <Text style={styles.cardCtaYouTubeText}>Watch</Text>
        </Pressable>
      ) : null}
    </View>
  );
...
function DetailsPanel({
  ...
  const todayTimeStyle = openNow ? styles.hoursTimeTodayOpen : styles.hoursTimeTodayClosed;
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
            {(() => {
              const img = String(featuredImageUrl ?? "").trim();
              if (!img) return null;
              return (
                <Image
                  source={{ uri: img }}
                  style={styles.detailsHeroImg}
                  resizeMode="cover"
                />
              );
            })()}
...
<Pressable
  ...
  onPress={() => {
    const url = `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(loc.latitude)},${encodeURIComponent(
      loc.longitude
    )}`;
    Linking.openURL(url);
  }}
      ...
      {h ? (
        <View style={styles.detailsBlock}>
          ...
          {days.map(([k, label]) => {
            ...
            return (
              <View key={String(k)} style={[styles.hoursRow, isToday && todayRowStyle]}>
                <Text style={[styles.hoursDay, isToday && todayDayStyle]}>{label}</Text>
                <Text style={[styles.hoursTime, isToday && todayTimeStyle]}>{txt}</Text>
              </View>
            );
          })}
        </View>
...
function ExpandableMultiSelectRow({
  label,
  value,
  expanded,
  onToggle,
        ...
        <View style={styles.optionsPanel}>
          ...
          {options.map((opt) => {
            ...
            return (
              ...
              >
                <View style={[styles.checkboxOuter, active && styles.checkboxOuterActive]}>
                  {active ? <Ionicons name="checkmark" size={14} color="#0b0f14" /> : null}
                </View>
                <Text style={[styles.optionText, active && styles.optionTextActive]}>{opt.label}</Text>
              </Pressable>
            );
